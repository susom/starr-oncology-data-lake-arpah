...
ARPA-H cohort included individuals with an existing record in STARR-OMOP and in the cancer registry (Neural Frame) or a present encounter of tumor board.

From the entire OMOP population there were 200,262 individuals who were eligible for ARPA-H cohort.

  - Neural Frame: Defined as patients within the Neural Frame who are permitted for research purposes (189k)

  - Tumor Board Encounter: Defined using EPIC patient encounter data (23k)
...
# Print YAML contents
print(sql_params)
file.exists("sql_params.yml")
getwd()  
list.files()  

yaml_lines <- readLines("sql_params.yml", warn = FALSE)
print(yaml_lines)  
...
tryCatch({
  sql_params <- yaml::read_yaml("sql_params.yml")
  print(sql_params)  # Check output
}, error = function(e) {
  message("Error reading YAML: ", e$message)
})
