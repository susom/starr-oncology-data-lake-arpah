...
*Release Version: February 2026 (IRB76049)*
...
den <- "/workspaces/starr-oncology-data-lake-arpah/src/feb_2026/sql_std/arpah_cohort/feb26/arpah_denominators_update.sql"
den_res <- fetch_data_from_sql_file(den, yaml_file_path, release = "feb_2026")
...
As of the February 2026 OMOP population, `r format( den_res$counts_pts[den_res$data_set=="Oncology OMOP (Cohort)"][1], big.mark = ",")` patients met the criteria for cancer cohort.
  ...
  - **OMOP Variant Occurrence:** The OMOP variant occurrence table captures records of genomic variants at the patient level providing a path to deidentified genomic data. Currently, it only contains variants identified via the STANFORD ACTIONABLE MUTATION PANEL FOR SOLID TUMORS (STAMP) test. The data is sourced from the Epic Genomics Suite, which incorporated STAMP tests starting in August of 2025.
    - [Variant Occurrence Metrics](variant_occ.qmd)

  - **OMOP Whole Slide Imaging:** The OMOP whole slide imaging table contains records of digitized pathology slides. This dataset is expected to eventually include images from three different sources. Currently, the table contains manually scanned slides that have been deidentified and converted into TIFF and JSON file formats. To ensure data quality and patient eligibility, only slides with order_proc_ids that can be linked to Epic Clarity are included, which prevents inclusion of HPP (Health Promotion and Prevention) or test patients.
    - [Whole Slide Imaging Metrics](whole_slide_imaging.qmd)
...
```{r, message=FALSE, warning=FALSE, results='hide'}
sql_ids <- "/workspaces/starr-oncology-data-lake-arpah/src/feb_2026/sql_std/arpah_cohort/feb26/arpah_overlap_metrics_updated.sql"
res <- fetch_data_from_sql_file(sql_ids, yaml_file_path, release = "feb_2026")
head(res)
...
### Source Codes:
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/blob/main/src/feb_2026/metrics_clinical.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/feb_2026/sql_std/arpah_cohort/feb26). 
