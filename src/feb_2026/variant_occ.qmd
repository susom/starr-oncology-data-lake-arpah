...
sql_comutation <- "/workspaces/starr-oncology-data-lake-arpah/src/feb_2026/sql_std/omop/variant_occ/feb26/variant_comutation.sql"
...
```{r comutation analysis, message=FALSE, warning=FALSE, fig.width=12, fig.height=10, incluude=FALSE}
# Get top 15 most frequently mutated genes
top_genes_comut <- comutation_data %>%
  count(gene_name, sort = TRUE) %>%
  head(15) %>%
  pull(gene_name)

# Filter to top genes and create patient-gene matrix
patient_gene_matrix <- comutation_data %>%
  filter(gene_name %in% top_genes_comut) %>%
  distinct(person_id, gene_name) %>%
...
gene_matrix <- patient_gene_matrix %>%
  select(-person_id) %>%
  as.matrix()

# Calculate correlation/co-occurrence
comut_matrix <- t(gene_matrix) %*% gene_matrix

# Convert to percentage of patients with both genes mutated
total_patients <- nrow(gene_matrix)
comut_pct <- comut_matrix / total_patients * 100

# Create heatmap data for plotly
gene_names <- rownames(comut_pct)
...
plot_ly(
  ...
  hovertemplate = paste(
    "Gene 1: %{x}<br>",
    "Gene 2: %{y}<br>",
    "Co-mutated in %{z:.1f}% of patients<br>",
    "<extra></extra>"
  ),
...
```{r top comutations table, message=FALSE, warning=FALSE, include=FALSE}
# Calculate pairwise co-mutations (excluding diagonal)
comut_pairs <- expand.grid(
  gene1 = gene_names,
  gene2 = gene_names,
  stringsAsFactors = FALSE
) %>%
  filter(gene1 < gene2) %>%  # Only upper triangle, avoid duplicates
  rowwise() %>%
  mutate(
    patients_with_both = comut_pct[gene1, gene2],
    patients_with_gene1 = comut_pct[gene1, gene1],
    patients_with_gene2 = comut_pct[gene2, gene2]
  ) %>%
  ungroup() %>%
  arrange(desc(patients_with_both)) %>%
  head(10) %>%
  mutate(
    patients_with_both = round(patients_with_both, 1),
    patients_with_gene1 = round(patients_with_gene1, 1),
    patients_with_gene2 = round(patients_with_gene2, 1)
  ) %>%
  rename(
    "Gene 1" = gene1,
    "Gene 2" = gene2,
    "% Both Mutated" = patients_with_both,
    "% Gene 1" = patients_with_gene1,
    "% Gene 2" = patients_with_gene2
  )
