...
color_palette <- c('#8dd3c7', '#ffffb3', '#bebada', '#fb8072', '#b3de69', '#fccde5')
...
- **Variants per Patient:** `r round(n_pts$n_variants / n_pts$n_patients, 1)`
:::

::: {.column width="60%"}
### 🧬 Data Components 
- **Unique Genes:** `r format(nrow(variant_genes), big.mark = ",")`
- **Variant Types:** `r format(nrow(variant_types), big.mark = ",")`
...
**Note:** These genes represent the most commonly mutated genes in cancer, many of which are therapeutically actionable.

### Variant Types

Distribution of variant types across the cohort.

```{r variant types, message=FALSE, warning=FALSE}
df_types <- variant_types %>% 
...
plot_ly(df_types,
        ...
        marker = list(colors = rep(color_palette, length.out = nrow(df_types)))) %>%
  layout(title = "Distribution of Variant Types")
```

**Variant Type Categories:**

- **SNV (Single Nucleotide Variant):** Single base pair changes
- **CNV (Copy Number Variant):** Amplifications or deletions
- **Indel:** Small insertions or deletions
- **MNV (Multiple Nucleotide Variant):** Multiple adjacent base changes
...
plot_ly(df_consequence,
  ...
  layout(
    xaxis = list(title = "Number of Variants"),
    yaxis = list(title = ""),
    title = "Top Molecular Consequences",
    margin = list(l = 150)
  )
```
...
plot_ly(df_specimen_source,
  ...
  layout(
    xaxis = list(title = "Number of Variants"),
    yaxis = list(title = ""),
    title = "Top 15 Specimen Sources",
    margin = list(l = 150)
  )
```

**Note:** Specimen source indicates the anatomic location from which the tested specimen was collected.
