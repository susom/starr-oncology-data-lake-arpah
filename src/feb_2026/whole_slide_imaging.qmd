...
The OMOP `_whole_slide_imaging` table contains records of digitized pathology slides, providing structured metadata and access paths to whole slide image files. This dataset enables comprehensive analysis of pathology imaging data within the OMOP framework.
...
```{r specimen_types_table, message=FALSE, warning=FALSE}
labels <- c(
...
```{r images_per_patient_chart, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r temporal_chart, message=FALSE, warning=FALSE}
library(ggplot2)
library(scales)
