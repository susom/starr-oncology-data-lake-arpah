...
df_plot <- df_plot %>%
  mutate(category = case_when(
    ...
    # Local Extension & Invasion
    grepl("INVASION|INVOLVED ADJACENT|LYMPHOVASCULAR|EXTRANODAL|SPREAD THROUGH AIR SPACES|VISCERAL PLEURA|DISTANT SITE", question, ignore.case = TRUE) ~ "Local Extension & Invasion",
...
medians <- df_plot %>%
 ...
 p <- ggplot(df_plot, aes(x = avg_score, fill = model_name)) +
   ...
   geom_density(aes(y=..count..*25), color = "grey", size = 1, alpha=0.7) +
   # median line with legend
     geom_vline(data = medians, aes(xintercept = median_score, color = model_name),
             linetype = "dashed", size = 0.5) +       # median line
  scale_fill_brewer(palette = "Set2") +
  scale_color_brewer(palette = "Set2") +
  theme_minimal() +
  labs(title = "Histogram and Distribution of Average Scores by Model",
       x = "Average Score",
       y = "Density",
       fill = "Model Name",
       color = "Model Name")+facet_wrap(~category)

p+facet_wrap(~category, scales="free_y", nrow=9, ncol=1)
```
```{r}
...
df_summary
 ...
 create_gt_table_v1(
    data = df_summary,
    columns = c("category", "gemini", "llama4", "openai_oss", "best_model"),
    labels = labels,
    footnote_text = ""
  )
```
