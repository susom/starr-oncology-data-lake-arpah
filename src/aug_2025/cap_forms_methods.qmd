...
```
The following boxplot displays the distribution of BERT scores across clinical categories, combining data from all three models (OpenAI OSS, Gemini, and LLAMA4). Categories are sorted by median performance, with the vertical dotted line at 0.85 representing the clinical acceptance threshold. Higher scores (further right) indicate better performance in extracting and structuring the relevant clinical information from pathology reports. Categories appearing higher in the plot demonstrate stronger overall performance across models.
...
plot_ly() %>%
  add_boxplot(
    data = success_analysis %>%
      group_by(category) %>%
      mutate(category_median = median(median)) %>%  # Calculate median of medians per category
      ungroup() %>%
      mutate(category = forcats::fct_reorder(category, category_median, .desc = TRUE)), # Sort by category median
    y = ~category,            
    x = ~median,             
    colors = c("#2ecc71"),   # dark green color
    text = ~sprintf("Median: %.2f", median),  # Simplified hover text
    hovertemplate = paste(
      "%{text}<extra></extra>"  # Just show the median
    ),
    jitter = 0.3,
    pointpos = 0,
    boxmean = TRUE          
  ...
  layout(
    title = list(
      text = "Performance Distribution by Clinical Category\n Across All Models",
      x = 0.5,
      xanchor = "center"
    ),
    xaxis = list(          
    ...
    shapes = list(          
      list(
        ...
        x1 = 0.85,
        y0 = -0.5,
        y1 = length(unique(success_analysis$category)) - 0.5,
        line = list(
          color = "grey",
          dash = "dot",
          width = 1
    ...
    showlegend = FALSE,
    margin = list(b = 50, r = 50, l = 150, t = 50)  
  )
```
...
plot_function <- function(input_data, model_var, show_legend = FALSE){
  ...
  pg <- plot_ly() %>%
    add_trace(
      ...
      #colors = c("High Performance" = "#55f499ff", 
            #"Medium Performance" = "#98c4e0ff", 
             # "Low Performance" = "#afefb1ff"), 
      type = "bar",
    ...
    layout(
      #title = list(text = paste("Model:",model_var), x = 0.5),  
...
Across the nine categories, OpenAI OSS achieved the highest overall success rate (56%), most frequently exceeding the 0.85 threshold. In contrast, Gemini demonstrated consistent performance across categories, ranking highest in Specimen & Procedure, and Staging compared to other models. Llama4 showed more balanced but slightly weaker results overall.
