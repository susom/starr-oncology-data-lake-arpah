...
Distribution of average scores (avg_score) across models is shown below. Histograms with dashed median lines indicate a minor differences in central tendency and variability between models. Summary statistics (mean, median, quartiles) are provided as follow.
...
The distributions of avg_score for each model were visualized using overlaid histograms with dashed lines indicating the median score per model within each category.
...
# Function to plot one or more categories
plot_categories <- function(df, category_names) {
  
  # filter for selected categories
  df_cat <- df %>% filter(category %in% category_names)
  
  # compute medians for selected categories
  medians_cat <- df_cat %>%
    group_by(model_name, category) %>%
    summarise(median_score = median(avg_score), .groups = "drop")
  
  # plot with facetting (one facet per category)
  p <- ggplot(df_cat, aes(x = avg_score, fill = model_name)) +
    geom_histogram(aes(y = ..count..), bins = 25, color = "black", alpha = 0.8, position = "identity") +
    geom_density(aes(y = ..count.. * 25), color = "grey", size = 1, alpha = 0.7) +
    geom_vline(data = medians_cat, aes(xintercept = median_score, color = model_name),
               linetype = "dashed", size = 0.5) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_brewer(palette = "Set2") +
    theme_minimal() +
    labs(
      title = paste("Histogram and Distribution of Avg Scores for:", 
...
# Example: plot multiple categories
plot_categories(df_plot, c("Staging", "Margins"))
...
p_box <- ggplot(datT, aes(x = model_name, y = avg_score, fill = model_name)) +
  ...
  theme_minimal() +
  labs(
    title = "Boxplot of Avg Scores for Tumor Characteristics",
    x = "Model Name",
