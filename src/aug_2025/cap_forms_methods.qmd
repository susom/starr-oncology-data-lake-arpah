...
## Category-Based Evaluation of LLMs for Lung Resection CAP Forms
...
df_plot <- df_plot %>%
  mutate(category = case_when(
    # Specimen & Procedure
    grepl("^(LUNG \\|\\| )?SPECIMEN ID|^PROCEDURE$|^SPECIMEN LATERALITY$|^NOTE$", question, ignore.case = TRUE) ~ "Specimen & Procedure",
...
In addition, a success threshold of 0.85 was established a priori. For each model–category combination, performance was classified into three tiers: high performance (≥0.85), medium performance (0.75–0.84), and low performance (<0.75). The success rate for each model was defined as the proportion of categories achieving high performance. Summary statistics included the count of successful categories, the total number of categories, the overall success rate, the mean of category-level medians, and a weighted average median adjusted for sample size. Results were visualized using interactive grouped bar plots to compare category-level medians by model, supplemented with a tabular summary for cross-model evaluation.
...
df_summary <- df_plot %>%
  group_by(model_name, category) %>%
  summarise(median_score = round(median(avg_score),2)) %>%
...
Across the nine categories, OpenAI_OSS achieved the highest overall success rate (56%) and the highest average median score (0.85), indicating that it most often met or exceeded the 0.85 threshold. In contrast, Gemini demonstrated the highest weighted average score (0.84), suggesting more consistent performance across categories, though it exceeded the threshold less frequently (33%). Llama4 showed balanced but slightly weaker results, with moderate success (44%) and an average median of 0.83. Taken together, OpenAI_OSS appears stronger in terms of peak performance across domains, while Gemini offers steadier but slightly lower results.
...
plot_ly() %>%
  ...
  layout(
    title = list(
      text = "Model Performance by Category<br></sup>",
      x = 0.5,
      xanchor = "center"
    ),
    xaxis = list(
