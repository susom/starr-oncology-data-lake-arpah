...
# 2. Box Plot with Jittered Points
```



```{r}
# Success Rate Analysis using model_stats_grp
success_analysis <- model_stats_grp %>%
  mutate(
    ...
    relative_performance = (median - 0.85) / 0.85 * 100  # Percentage difference from threshold
  ) %>%
  arrange(desc(median))
...
plot_ly() %>%
  ...
  # Add shadow effect for the threshold line
  add_segments(
    x = -1, xend = length(unique(success_analysis$category)),
    y = 0.845, yend = 0.845,           # Slightly below main line
    line = list(
      color = "rgba(255, 0, 0, 0.2)",  # Very transparent red
      width = 4                         # Wider for shadow effect
    ),
    showlegend = FALSE
  ) %>%
  layout(
    title = list(
      text = "Model Performance by Category<br><sup>Threshold = 0.85 (Sample Size Indicated)</sup>",
      x = 0.5,
      xanchor = "center"
    ),
    xaxis = list(
...
model_wide <- df_plot %>%
  select(model_name, category, avg_score) %>%
  pivot_wider(
    names_from = model_name,
    values_from = avg_score
  )

correlation_matrix <- cor(model_wide[,-1], use = "complete.obs")
...
plot_ly(
  ...
  layout(
    title = "Model Score Correlation Matrix",
    xaxis = list(title = "Model"),
