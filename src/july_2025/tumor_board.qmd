...
p <- ggplot(dt, aes(x = year, y = n_tb_encounters, color = flag, group = flag)) +geom_point(aes(y = n_tb_encounters), size = 3)+
  geom_smooth(method = "loess", linewidth = 0.5, alpha = 0.7, se=FALSE) + # Add smoothed lines
  theme_minimal() +
  labs(
...
library(forcats)
df_bar_ana <- res_ana %>% filter (unique_person_count>45)%>%
...
plot_ly(df_bar_ana,
      ...
      #  textposition = "outside",  
       customdata = ~percent,
        ...
        colors = rev(RColorBrewer::brewer.pal(n = length(unique(df_bar_ana$anatomic_site_source_value)), "Set3")),
        hovertemplate = paste(
          "Anatomic: %{y}<br>",
...
plot_ly(df_bar,
      ...
      #  textposition = "outside",  
       customdata = ~percent,
        ...
        colors = rev(RColorBrewer::brewer.pal(n = length(unique(df_bar$modality_source_value)), "Set3")),
        hovertemplate = paste(
          "Anatomic: %{y}<br>",
          "Patient Count: %{x:,.0f}<br>",
          "Patient Percentage: %{customdata:.1f}%<br>",
