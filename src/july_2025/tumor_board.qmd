...
yaml_file_path <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql_params.yml"
```
...
tb_sql <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_person_metrics.sql"
...
## basic counts 
sql_thr<- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/image_occ_thoracic/image_occ_person_thr.sql"
res_den_thr<- fetch_data_from_sql_file(sql_thr, yaml_file_path)
head(res_den_thr)
```

As of July 2025, From `r format(res_den_thr$counts[1], big.mark = ",")` patients with thoracic cancer, `r format(tb_den$patient_count[1] , big.mark = ",")` also had a Tumor Board encounter. 
```{r time trend, message=FALSE, warning=FALSE, results='hide'}
# time trend
tb_time <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_over_time.sql"
...
p <- ggplot(dt, aes(x = year, y = n_tb_encounters, color = flag, group = flag)) +
  geom_point(aes(
    text = paste0(
      "Year: ", year,
...
# Parameters
sql_overlap <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_overlap.sql"
...
# Compute percentages
#res$fraction <- res$total_patients / res$all_tb
#res$ymax <- cumsum(res$fraction)
#res$ymin <- c(0, head(res$ymax, n = -1))
#res$label_pos <- (res$ymax + res$ymin) / 2
#res$label <- paste0(res$flag, "\n", round(res$fraction * 100), "%")
```
```{r}
ggplot(res, aes(x = reorder(flag, total_patients), y = total_patients, fill = flag)) +
  geom_col(width = 0.8) +
  coord_flip() +
  labs(x = NULL, y = "Patient Count", title = "Tumor Board Subgroup Sizes") +
  geom_text(aes(label = total_patients), hjust = -0.1) +
  theme_minimal() +
...
```{r demographic, message=FALSE, warning=FALSE, results='hide'}
# Parameters
sql_demg <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_demographic_all.sql"
...
# Parameters
sql_case <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_case_status.sql"
...
# Parameters
sql_img <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_imag_occ.sql"
...
### Anatomic Sites
```{r, message=FALSE, warning=FALSE, results='hide'}
sql_image_an <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_imag_occ_anatomic.sql"
...
# Parameters
sql_img_mod <- "/workspaces/starr-oncology-data-lake-arpah/src/july_2025/sql/tumor_board/tumor_board_thoracic_imag_occ_modality.sql"
