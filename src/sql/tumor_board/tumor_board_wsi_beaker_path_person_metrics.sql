...
all_flag as (select * from `@oncology_dev.@oncology_tmp.onc_all__cancer_flags`),
...
inner join wsi_beaker_path_patients wsi on person.person_source_value = CONCAT(FORMAT('%s | %s', wsi.pat_mrn_id, CAST(cast(wsi.birth_date AS date) AS string format 'yyyy-mm-dd')))