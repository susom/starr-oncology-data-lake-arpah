...
nf_den <- fetch_data_from_sql_file(sql_den, yaml_file_path, release = "may_2026")
nf_den
```
...
case <- fetch_data_from_sql_file(sql_nf, yaml_file_path, release = "may_2026")
head(case)
...
thr_case <- fetch_data_from_sql_file(sql_thr_case, yaml_file_path, release = "may_2026")
head(thr_case)
...
thr_den=fetch_data_from_sql_file(sql_thr, yaml_file_path, release = "may_2026")
thr_den
```
```{r,message=FALSE, warning=FALSE, results='hide'}
# Calculate counts and percentages using unique person counts
...
thr_den_c=fetch_data_from_sql_file(sql_thr_c, yaml_file_path, release = "may_2026")
thr_den_c
## denominators ##
sum_thr_c=thr_den_c$unique_thoracic_cancer_pts
sum_thr=thr_den$unique_thoracic_cancer_pts
```
As of May 2026, there are  `r format(thr_den$unique_thoracic_cancer_pts, big.mark = ",")` unique thoracic patients with `r format(case_thr$Percentage[case_thr$Status=="Completed"], big.mark = ",")` % completed cases. 
...
thr_yr <- fetch_data_from_sql_file(sql_thr_yr, yaml_file_path, release = "may_2026")
...
thr_his <- fetch_data_from_sql_file(sql_thr_his, yaml_file_path, release = "may_2026")
...
thr_stg <- fetch_data_from_sql_file(sql_stg, yaml_file_path, release = "may_2026")
thr_stg
...
thr_tx <- fetch_data_from_sql_file(sql_tx, yaml_file_path, release = "may_2026")
...
recurrence <- read_csv("/workspaces/starr-oncology-data-lake-arpah/src/may_2026/data/recurrenceType_category.csv")
...
thr_rec <- fetch_data_from_sql_file(sql_rec, yaml_file_path, release = "may_2026")
...
thr_soh <- fetch_data_from_sql_file(sql_soh, yaml_file_path, release = "may_2026")
...
thr_smk <- fetch_data_from_sql_file(sql_sm_omop, yaml_file_path, release = "may_2026")
...
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/may_2026/neural_frame.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/neural_frame). 
