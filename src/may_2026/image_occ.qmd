...
library(forcats)
sql_file_path_image_mod <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/omop/image_occ/feb26/image_occ_modality.sql"
res_img_mod <- fetch_data_from_sql_file(sql_file_path_image_mod, yaml_file_path, release = "may_2026")
head(res_img_mod)
...
path_img <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/omop/image_occ/feb26/image_occ_person.sql"
pt_img <- fetch_data_from_sql_file(path_img, yaml_file_path, release = "may_2026")
pt_img
den_series=as.numeric(pt_img$counts)[2]
den_pts=as.numeric(pt_img$counts)[1]
```

As of May 2026, this table included images for  `r format(pt_img$counts[1], big.mark = ",")` patients with `r format(pt_img$counts[2], big.mark = ",")` series. **Series** are defined as individual imaging acquisitions within a study.


```{r, message=FALSE, warning=FALSE, results='hide'}
## basic counts 
sql_file_path_image<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/omop/image_occ/feb26/image_occ_basic.sql"
res<- fetch_data_from_sql_file(sql_file_path_image, yaml_file_path, release = "may_2026")
head(res)
...
sql_file_path_image_an <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/omop/image_occ/feb26/image_occ_anatomic.sql"
res_ana <- fetch_data_from_sql_file(sql_file_path_image_an, yaml_file_path, release = "may_2026")
head(res_ana)
...
```{r, message=FALSE, warning=FALSE}
library(DT)
res_ana=res_ana %>% filter(!is.na(anatomic_site))
res_ana=res_ana %>% filter(anatomic_site!="null")
...
res_den_thr<- fetch_data_from_sql_file(sql_thr, yaml_file_path, release = "may_2026")
head(res_den_thr)
```

As of May 2026, from `r format(res_den_thr$counts[res_den_thr$variable_name=="Number of unique persons with thoracic cancer"], big.mark = ",")` patients with thoracic cancer, `r format(res_den_thr$counts[res_den_thr$variable_name=="Number of unique persons in image_occurrence table"], big.mark = ",")` had imaging data. 
...
## modality
sql_thr_mod<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/image_occ_thoracic/image_occ_modality_thr.sql"
res_thr<- fetch_data_from_sql_file(sql_thr_mod, yaml_file_path, release = "may_2026")
head(res_thr)
res_thr
...
```{r, message=FALSE, warning=FALSE, results='hide'}
## anatomic
sql_thr_ana<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/image_occ_thoracic/image_occ_anatomic._thr.sql"
res_ana_thr<- fetch_data_from_sql_file(sql_thr_ana, yaml_file_path, release = "may_2026")
head(res_ana_thr)
...
### Source Code:
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/may_2026/image_occ.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/omop/image_occ).
The queries regarding thoracic cancer can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/thoracic/feb26/image_occ_thoracic).
