...
overlap_plot_data <- bind_rows(
  ...
  group_by(release, data_set, month) %>%
  summarise(counts_pts = sum(counts_pts, na.rm = TRUE), .groups = "drop") %>%
  mutate(
...
- February 2026 combines both Philips ISPM and OMOP _variant_occurrence sources (deduplicated)
