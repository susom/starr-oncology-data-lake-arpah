...
**Analysis Data Sources:** This analysis reflects the raw PHI data from February 2025 to May 2026.
...
plot_ly(
  ...
  layout(
    ...
    annotations = list(
      list(
        text = "* Feb 2026 Philips ISPM includes OMOP variant_occurrence data",
        xref = "paper",
        yref = "paper",
        x = 0,
        y = -0.15,
...
- The Tumor Board definition was updated in May 2025. February 2025 data uses the original definition based on encounter type, while May 2025 to February2026 releases use an enhanced definition based on the OMOP visit_occurrence table, where the source value for the visit contains the phrase 'tumor board'.

- Feb 2026 Philips ISPM/Genomic Data includes patients from OMOP _variant_occurrence table combined with Philips ISPM data.
...
**Imaging Coverage Growth**: The imaging dataset has expanded dramatically from approximately 93k patients in February 2025 to over 199k patients by February 2026, representing a substantial increase in multi-modal data availability for cancer research.
...
# Add individual test type traces for Feb 2026 (hidden by default)
for (i in seq_len(nrow(feb26_test_types_plot))) {
  test_data <- feb26_test_types_plot[i, ]
  p <- p %>%
    add_trace(
      x = test_data$month_label,
      y = test_data$test_type_counts,
      type = 'scatter',
      mode = 'markers',
      name = paste0(test_data$test_type, " (Feb 2026)"),
      marker = list(
        size = 12,
      ...
      text = paste0(
        "<b>", test_data$test_type, "</b>",
        "<br>Feb 2026",
        "<br>Tests: ", scales::comma(test_data$test_type_counts),
        "<br><i>Deduplicated across both sources</i>"
      ),
      hoverinfo = "text",
      visible = "legendonly"
    )
...
p %>%
  layout(
    ...
    annotations = list(
      ...
      list(
        text = "Note: Feb 2026 combines both Philips ISPM and OMOP variant_occurrence sources (deduplicated)",
        xref = "paper",
        yref = "paper",
        x = 0,
        y = -0.15,
...
- February and May 2026 combines both Philips ISPM and OMOP _variant_occurrence sources (deduplicated)
