...
tb_den<- fetch_data_from_sql_file(tb_sql, yaml_file_path, release = "may_2026")
tb_den
...
As of May 2026, From `r format(res_den_thr$counts[res_den_thr$variable_name=="Number of unique persons with thoracic cancer"], big.mark = ",")` patients with thoracic cancer, `r format(tb_den$n_pts[1] , big.mark = ",")` also had a Tumor Board encounter. 
...
dt <- fetch_data_from_sql_file(tb_time, yaml_file_path, release = "may_2026")
head(dt)
```
...
res<- fetch_data_from_sql_file(sql_overlap, yaml_file_path, release = "may_2026")
...
res <- fetch_data_from_sql_file(sql_demg, yaml_file_path, release = "may_2026")
```
...
case <- fetch_data_from_sql_file(sql_case, yaml_file_path, release = "may_2026")
case
...
tb_img<- fetch_data_from_sql_file(sql_img, yaml_file_path, release = "may_2026")
tb_img
```
As of May 2026, `r format(tb_img$patient_count[1], big.mark = ",")` had image occurrence procedures.
...
res_ana <- fetch_data_from_sql_file(sql_image_an, yaml_file_path, release = "may_2026")
head(res_ana)
...
tb_img_mod<- fetch_data_from_sql_file(sql_img_mod, yaml_file_path, release = "may_2026")
tb_img_mod
```
...
### Source Codes:
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/may_2026/tumor_board_updated.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/tumor_board/feb26). 
