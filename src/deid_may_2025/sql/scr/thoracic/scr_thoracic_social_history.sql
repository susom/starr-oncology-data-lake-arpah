...
scr_data as
(select scr.* from scr
inner join person p ON p.person_id = scr.person_id
inner join dx on scr.person_id=dx.person_id
),
smk_stat as (select naaccrtobaccodescription, count(distinct(person_id)) as counts, 'smoking status' as flag
from scr_data
group by 1
order by 1),
alc_stat as (select naaccralcoholdescription, count(distinct(person_id)) as counts, 'alcohol status' as flag
from scr_data
group by 1
order by 1)
