...
count(distinct person.person_id) patient_count
  from
  person
inner join scr_thoracic_patients on person.person_id = concat(scr_thoracic_patients.cleaned_nf_mrn, ' | ', scr_thoracic_patients.cleaned_nf_dob)
inner join tumor_board_patients tb on person.person_id =tb.person_id
