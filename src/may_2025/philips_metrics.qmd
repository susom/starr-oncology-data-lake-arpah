...
thr_abr
```
From `r format(thr_den$unique_thoracic_cancer_pts, big.mark = ",")` unique thoracic patients, `r format(thr_abr$n_pts, big.mark = ",")` had aberration cases. (how many were tested)?
...
```
## Distribution of Pathogenicity Types
```{r, message=FALSE, warning=FALSE}
# Prepare the data
