...
# Prepare the plot
p <- gene_ch %>%
  filter(n_pts > 100, !is.na(hgnc_gene)) %>%
  ggplot(aes(x = reorder(hgnc_gene, pts_percent), y = pts_percent, fill = chrom, text = paste("Gene:", hgnc_gene, "<br>Count:", n_pts))) +
  ...
  labs(
    x = "HGNC Gene",
    y = "% Patient",
    title = "Top Genes Distribution by Chromosomes"
  ) +
  theme_minimal()
...
```{r, message=FALSE, warning=FALSE}
  ...
  geom_jitter(
    ...
    height = 0.3   # Vertical jitter amount
  ) +
