...
In the OMOP Common Data Model (CDM), the "Image_occurrence" table, alongside the "Image_feature" table, provides a structured way to represent medical imaging events and features, bridging the gap between imaging and observational research by integrating image-based measurements into the CDM. 
...
At Stanford, this table has been extended to include a link to the note table, allowing integration of imaging data with relevant clinical documentation. (edited) 
```{r, message=FALSE, warning=FALSE, results='hide'}
