...
```
## Staging 
The following table summarizes the staging status of cancer patintes. Stages include: 
```{r staging, message=FALSE, warning=FALSE}
...
```
## Summary of Staging of Thoracic Cancer Patinets
The following table summarizes the staging status of thoracic patients with complete cases. 
...
```{r load recurrence, message=FALSE, warning=FALSE }
library(readr)
...
```
```{r thoracic recurrence, message=FALSE, warning=FALSE, results='hide'}
