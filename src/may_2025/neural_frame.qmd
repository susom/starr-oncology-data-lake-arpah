...
```{r histology barplot, message=FALSE, warning=FALSE}
    thr_his=thr_his %>% filter(histology!="Other/Not Specified")
...
plot_ly(summary_plot,
  ...
  layout(
    title = "Distribution of Thoracic Cancer Cases by Histology Types",
...
The following dual-layered classification system by specific histology and then by broader clinical grouping (NSCLC, SCLC, Other Specified, and Other unspecified) represents the thoracic cancer distribution by histology grouping. 
...
### Detailed Pie Chart
Detailed distribution of thoracic cancer subcategories are shown below.
...
## Table Summary of Histology Groups by ICDO3 Codes
