...
This dual-layered classification system—first by specific histology and then by broader clinical grouping (NSCLC, SCLC, Other Specified, Other Unspecified)—helps in organizing thoracic cancer data for research and reporting.
...
sunburst_data <- thr_his %>% filter (patient_count>10)%>%
  group_by(histology, histology_group) %>%
  summarize(value = sum(patient_count)) %>%
  ungroup()

# Calculate total for percentages
total_count <- sum(sunburst_data$value)
...
datatable(histology_summary,
          colnames = c("Main Category", "Subcategory", "Patient Count", "Percentage (%)"),
          options = list(
            pageLength = 10,
            dom = 'Bfrtip',
            ordering = TRUE
...
```{r, message=FALSE, warning=FALSE}
# Create main summary
thr_his_res=thr_his %>% filter (patient_count>10)
...
tab_his<- thr_his_summary %>% select (histology_group, histologicTypeIcdO3Description, histology_codes, patient_count, percentage)
...
### Treatment
The following table summarizes the distribution of thoracic cancer treatments including chemotherapy, radiation therapy, and surgery across the patient cohort.
...
thr_tx_long <- thr_tx %>%
  ...
  mutate(
    percentage = round(100 * total_patients / sum(total_patients), 1),
...
```
### Social History 
From  `r format(thr_den$unique_thoracic_cancer_pts, big.mark = ",")` unique thoracic patients, the majority didn't have any records of smoking status and alcohol use. 
...
smoking_data <- thr_soh[!grepl("alcohol", tolower(thr_soh$Description)), ]
alcohol_data <- thr_soh[grepl("alcohol", tolower(thr_soh$Description)), ]
```
