...
summary_plot <- thr_his %>%
  ...
  summarise(
    ...
    pct_of_total = round(100 * count / sum(thr_his$patient_count), 1),
    .groups = 'drop'
  ) %>%
  arrange(desc(count)) %>%
  mutate(
...
The following dual-layered classification system—first by specific histology and then by broader clinical grouping (NSCLC, SCLC, Other Specified, Other Unspecified) represents the thoracic cancer distribution by histology grouping. 
...
datatable(histology_summary,
          colnames = c("Histology", "Subcategory", "Patient Count", "Percentage (%)"),
          options = list(
            pageLength = 10,
            dom = 'Bfrtip',
            ordering = TRUE
...
DT::datatable(tab_his,
              colnames = c("Histology","Histology Group", "Original Histology", "ICD-O-3 Codes", "Patient Count", "Percentage (%)"),
              options = list(
                pageLength = 10,
                searching = TRUE,
                ordering = TRUE,
