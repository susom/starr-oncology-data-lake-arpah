...
The donut chart below shows how patients are distributed across different data sources:
...
donut_data <- den_res %>%
  mutate(
    ...
    label = paste0(data_set, "\n", 
                  format(counts_pts, big.mark=","), " patients\n", 
                  "(", round(percentage, 1), "%)"))
...
upset(
  fromList(list(
    ...
    "Philips ISBM" = overlap_matrix$person_id[overlap_matrix$`Philips ISBM`]
  )),
  sets = c("Oncology OMOP", "Image Occurrence", "Neural Frame", "Philips ISBM"),
  order.by = "freq",
  mainbar.y.label = "Number of Patients",
  sets.x.label = "Total Patients in Source",
  point.size = 3,
  line.size = 1,
  text.scale = 1.2,
  mb.ratio = c(0.6, 0.4),
  keep.order = TRUE,
  matrix.color = "#6495ED",  # Using an even lighter blue color
  main.bar.color = "#6495ED",
  sets.bar.color = "#6495ED",
