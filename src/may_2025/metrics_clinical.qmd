...
```
```{r, include='FALSE'}
# Create the UpSet plot with improved styling
library(ggplot2)
library(ggupset)
library(dplyr)
library(purrr)
...
all_sources <- c("Oncology OMOP", "Neural Frame", "Philips ISPM", "Image Occurrence", "Tumor Board")

# Generate all possible combinations (power set, except empty set)
all_combos <- map(1:length(all_sources), ~combn(all_sources, ., simplify = FALSE)) %>%
  unlist(recursive = FALSE)

# Convert to list-column format for joining
all_combos_df <- tibble(
  sources = lapply(all_combos, sort)
)

# Your observed data
upset_data <- res %>%
  group_by(person_id) %>%
  summarize(sources = list(sort(data_set)), .groups = "drop") %>%
  count(sources, name = "n")

# Join with all possible combinations, fill missing with zero
upset_data_full <- all_combos_df %>%
  left_join(upset_data, by = "sources") %>%
  mutate(n = replace_na(n, 0)) %>%
  arrange(desc(n))

# Plot
ggplot(upset_data_full, aes(x = sources)) +
  geom_bar(aes(y = n), stat = "identity", fill = "#8B0000", width = 0.9) +
  scale_x_upset(
    sets = all_sources,
    n_intersections = 32
  ) +
  geom_text(aes(y = n, label = n), vjust = -0.4, size = 4) +
  theme_minimal() +
  theme(
  ...
  labs(
    title = "Patient Distribution Across All Data Source Combinations",
