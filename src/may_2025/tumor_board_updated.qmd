...
```
Please note that the previous Tumor Board definition has been updated and the previous released metrics are presented here:
...
## basic counts 
sql_thr<- "/workspaces/starr-oncology-data-lake-arpah/src/may_2025/sql/image_occ_thoracic/image_occ_person_thr.sql"
...
p <- ggplot(dt, aes(x = year, y = n_tb_encounters, color = data, group = data)) +
  ...
  geom_smooth(method = "loess", linewidth = 0.5, alpha = 0.7, se = FALSE) +
    geom_vline(xintercept = 2020, linetype = "dashed", color = "grey", size = 0.5) +  # Add vertical line for year 2020
