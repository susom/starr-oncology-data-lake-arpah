...
-- Placeholders: @oncology_omop_feb, @oncology_neuralframe_feb, etc.
-- =========================================
...
WITH vital_concepts AS (
    ...
    */
    SELECT conc.*
    FROM `@oncology_omop.concept_ancestor` ca
    INNER JOIN `@oncology_omop.concept` conc
...
smoking_concepts AS (
    SELECT conc.*
    FROM `@oncology_omop.concept_relationship` cr 
    INNER JOIN `@oncology_omop.concept` conc
      ON cr.concept_id_2 = conc.concept_id
    WHERE cr.concept_id_1 IN (
        SELECT concept_id
        FROM `@oncology_omop.concept`
...
FROM `@oncology_omop.visit_occurrence`

UNION DISTINCT 

SELECT 'total_pt_gt_12', 
       COUNT(DISTINCT per.person_id)
FROM `@oncology_omop.person` per
JOIN `@oncology_omop.visit_occurrence` vis
...
FROM (
    SELECT person_id
    FROM `@oncology_omop.measurement` meas
    JOIN `@oncology_omop.concept` conc
      ON meas.measurement_concept_id = conc.concept_id
     AND conc.vocabulary_id = 'LOINC'
    UNION DISTINCT
    SELECT person_id
    FROM `@oncology_omop.observation` obs
    JOIN `@oncology_omop.concept` conc
...
FROM `@oncology_omop.drug_exposure` drug
JOIN `@oncology_omop.concept` conc
...
SELECT 'uniq_pt_icd_dx',
       COUNT(DISTINCT person_id)
FROM `@oncology_omop.condition_occurrence`
WHERE condition_concept_id IN (
    SELECT cr.concept_id_2
    FROM `@oncology_omop.concept` c1
    JOIN `@oncology_omop.concept_relationship` cr
...
SELECT 'uniq_pt_icd_proc',
       COUNT(DISTINCT person_id)
FROM `@oncology_omop.procedure_occurrence`
...
FROM (
    ...
    UNION DISTINCT
    SELECT person_id
    FROM `@oncology_omop.observation`
...
SELECT 'uniq_pt_any_insurance_value',
       COUNT(DISTINCT person_id)
FROM `@oncology_omop.payer_plan_period`;
