...
death_table <- death_res %>%
  mutate(
    month = case_when(
      month == "May" ~ "May 2025",
      month == "August" ~ "August 2025",
      month == "November" ~ "November 2025",
      TRUE ~ month
    ),
...
The mortality tracking shows the cumulative count of patients with death records from either OMOP clinical data (death_date) or the Stanford Cancer Registry (vital status = "Dead"). The trend reflects both actual mortality events and improvements in data capture and linkage across releases.
...
```{r stacked_bar_plot, message=FALSE, warning=FALSE}
library(plotly)
library(dplyr)

# Create stacked bar chart to show proportions
plot_ly(
  data = tb_plot_data_ordered,
  x = ~month_label,
  y = ~patients,
  ...
  layout(
    title = "Tumor Board Data Source Composition Across Releases",
    xaxis = list(
      title = "Release Month",
      categoryorder = "array",
      categoryarray = c("February", "May", "August", "November")
...
```{r philips_tests_plot, message=FALSE, warning=FALSE, include=FALSE}
