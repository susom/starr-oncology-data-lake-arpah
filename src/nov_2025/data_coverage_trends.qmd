...
```
### Data Sources Overlap Across Releases
...
```{r overlap summary, message=FALSE, warning=FALSE}
library(tidyr)

# Data already aggregated - just pivot it
overlap_summary <- overlap_res %>%
  select(data_set, counts_pts, month) %>%
  pivot_wider(
    names_from = month,
    values_from = counts_pts,
    values_fill = 0
  ) %>%
  arrange(desc(aug))
...
knitr::kable(
  overlap_summary,
  col.names = c("Data Source", "February", "May", "August"),
  caption = "Unique patient counts by data source across releases",
