...
*Release Version: August 2026 (IRB76049)*
...
den <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/arpah_cohort/nov25/arpah_denominators_update.sql"
den_res <- fetch_data_from_sql_file(den, yaml_file_path, release = "aug_2026")
...
As of the August 2026 OMOP population, `r format( den_res$counts_pts[den_res$data_set=="Oncology OMOP (Cohort)"][1], big.mark = ",")` patients met the criteria for cancer cohort.
  ...
  - **Neural Frame:** Defined as research-eligible patients with case records in Neural Frame (aka Stanford Cancer Registry).
    ...
    - [Neural Frame Event Metrics](neural_frame_event_table.qmd)
  ...
  - **OMOP Episode Table:**  The OMOP episode table captures records of clinical episodes, which are defined as clusters of healthcare events related to a specific condition or treatment course. Episode table is currently populated only with episodes representing treatment plans for patients in the oncology cohort.
    - [Episode Table Metrics](episode_table.qmd)


  - **External Death Records:** The external death records table captures records of deaths reported from external sources, such as state death registries or other healthcare institutions. This table is designed to complement the standard OMOP death table, which contains only deaths confirmed within the EHR (Epic).
    - [External Death Records Metrics](external_death_records.qmd)


  - **Data Coverage Trends:** Longitudinal view of data evolution across all platforms from February 2025 to August 2026.
    - [Data Coverage Trends](deid_data_coverage_trends.qmd)
...
```{r, message=FALSE, warning=FALSE, results='hide'}
sql_ids <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/arpah_cohort/nov25/arpah_overlap_metrics_updated.sql"
res <- fetch_data_from_sql_file(sql_ids, yaml_file_path, release = "aug_2026")
head(res)
...
### Source Codes:
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/blob/main/src/deid_aug_2026/metrics_clinical.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std/arpah_cohort/nov25). 
