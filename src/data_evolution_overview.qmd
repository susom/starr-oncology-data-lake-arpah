...
digraph vista_data_lake {
  ...
  # Graph attributes
  graph [layout = dot, rankdir = TB, bgcolor = '#FFFFFF', fontname = 'Arial']
  
  # Node definitions with styling
  node [shape = box, style = filled, fontname = 'Arial', fontsize = 11]
  
  # Top level - Data Lake
  DL [label = 'VISTA Oncology Data Lake\n(217K+ Patients)', fillcolor = '#2E86AB', fontcolor = white, fontsize = 14, shape = box, width = 4]
  
  # Core Components
  OMOP [label = 'OMOP CDM 5.4.2\nClinical Data', fillcolor = '#8dd3c7', fontsize = 12]
  NF [label = 'NeuralFrame\nRegistry Data', fillcolor = '#ffffb3', fontsize = 12]
  IMG [label = 'Image Occurrence\n1.6M+ Patients', fillcolor = '#bebada', fontsize = 12]
  PHILIPS [label = 'Philips ISPM\nMolecular Testing', fillcolor = '#fb8072', fontsize = 12]
  
  # OMOP sub-components
  DEMO [label = 'Demographics\nEncounters', fillcolor = '#A8DADC', fontsize = 10]
  CLIN [label = 'Diagnoses\nProcedures\nMedications', fillcolor = '#A8DADC', fontsize = 10]
  LABS [label = 'Lab Results\n(LOINC)', fillcolor = '#A8DADC', fontsize = 10]
  NOTES [label = 'Clinical Notes', fillcolor = '#A8DADC', fontsize = 10]
  TB [label = 'Tumor Board\nEncounters', fillcolor = '#A8DADC', fontsize = 10]
  
  # NeuralFrame sub-components
  NFDIAG [label = 'Cancer Diagnoses', fillcolor = '#FFFACD', fontsize = 10]
  NFTRT [label = 'Treatments', fillcolor = '#FFFACD', fontsize = 10]
  NFOUT [label = 'Outcomes', fillcolor = '#FFFACD', fontsize = 10]
  NFMISC [label = 'Misc', fillcolor = '#FFFACD', fontsize = 10]

  # Imaging sub-components
  IMGMOD [label = 'CT/PET\nMRI\nX-Ray', fillcolor = '#D8BFD8', fontsize = 10]
  IMGMETA [label = 'DICOM Metadata\nAnatomic Regions', fillcolor = '#D8BFD8', fontsize = 10]
  
  # Philips sub-components
  STAMP [label = 'STAMP\nHeme-STAMP', fillcolor = '#FFB6C1', fontsize = 10]
  F1 [label = 'FoundationOne', fillcolor = '#FFB6C1', fontsize = 10]
  STAMPMETA [label = 'STAMP Add-On\nMetadata', fillcolor = '#FFB6C1', fontsize = 10]
  
  # Special Datasets
  SPECIAL [label = 'Specialized Datasets', fillcolor = '#F4A460', fontsize = 12, shape = box]
  HIPPO [label = 'HIPPO Benchmark\n1,394 Notes', fillcolor = '#FFA07A', fontsize = 10]
  CAP [label = 'CAP Forms\nPathology', fillcolor = '#FFA07A', fontsize = 10]
  EPIC [label = 'Epic Media Server\nPDFs', fillcolor = '#FFA07A', fontsize = 10]
  
  # Research Tools
  TIDE [label = 'Tide 2.0\nAgentic AI', fillcolor = '#6A4C93', fontcolor = white, fontsize = 12, shape = ellipse]
  
  # Connections
  DL -> {OMOP NF IMG PHILIPS SPECIAL TIDE}
  
  OMOP -> {DEMO CLIN LABS NOTES TB}
  NF -> {NFDIAG NFTRT NFOUT NFMISC}
  IMG -> {IMGMOD IMGMETA}
  PHILIPS -> {STAMP F1 STAMPMETA}
  SPECIAL -> {HIPPO CAP EPIC}
  
  # Styling for edges
  edge [color = '#457B9D', arrowsize = 0.8]
}
")
```
...
release_summary <- tibble::tribble(
  ...
  "May 2025",
  "OMOP CDM 5.3.1",
  "Full registry (diagnoses, treatments, outcomes, miscellaneous)",
  "Initial release (STAMP, Heme-STAMP, FoundationOne)",
  "Full NeuralFrame registry, Image occurrence data, Philips ISPM molecular testing",
  ...
  "STAMP Add-On metadata, Epic Media Server PDFs",
  "No schema changes; continued data enrichment",
  "HIPPO Benchmark (1,394 annotated notes), CAP Forms (lung resection pathology)",
  
  "November 2025",
  "OMOP CDM 5.4.2",
  "Continued coverage",
  "Expanded genomic testing coverage",
  "Expanded genomic testing, Agentic Tide integration",
  "Major upgrade: Updated vocab, enhanced field linking, JSON-formatted source values",
  "Enhanced ETL pipeline with standardized filtering (events from 2000+, flexible visit requirements)"
)
...
- **stanford_patient_uid**: Stanford-specific unique patient identifier used in NeuralFrame and Philips ISPM datasets which is concatination of MRN and DOB
- **MRN, DOB**: Medical Record Number and Date of Birth combination (used in February 2025 before standardization)
- **jc_uid**: De-identified note identifier in HIPPO benchmark dataset
- **anon_id**: Anonymized patient identifier in CAP pathology forms

**Additional Linkage Fields:**

- Image Occurrence table also includes **note_id**, **visit_occurrence_id**, and **_accession_number** for linking to clinical notes, visits, and imaging studies
