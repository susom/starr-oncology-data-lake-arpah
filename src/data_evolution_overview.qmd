...
timeline <- tribble(
  ...
  "OMOP Image Occurrence",            "OMOP",         "2025-05-01",   "2026-08-31",   "Active",
  ...
  "Variant Occurrence (Multi-panel)", "OMOP",         "2026-08-01",   "2026-08-31",   "Active",
  ...
  "Philips ISPM",                     "Molecular",    "2025-05-01",   "2026-05-01",   "Discontinued",
  "STAMP Add-On Metadata",            "Molecular",    "2025-08-01",   "2026-08-31",   "Active",
  # Specialized Datasets
  "HIPPO Benchmark",                  "Specialized",  "2025-08-01",   "2026-08-31",   "Active",
  "CAP Forms",                        "Specialized",  "2025-08-01",   "2026-08-31",   "Active",
  "Epic Media Server PDFs",           "Specialized",  "2025-08-01",   "2026-08-31",   "Active"
) %>%
  mutate(
    Start = as.Date(Start),
    End   = as.Date(End),
...
for (i in seq_len(nrow(timeline))) {
  row   <- timeline[i, ]
  color <- category_colors[as.character(row$Category)]
  is_active <- row$Status == "Active"

  fig <- fig %>% add_trace(
    type   = "scatter",
    mode   = "lines",
    x      = c(row$Start, row$End),
    y      = c(as.character(row$Source), as.character(row$Source)),
    line   = list(
      color = color,
      width = 22,
      dash  = if (is_active) "solid" else "dot"
    ),
    opacity = if (is_active) 1 else 0.5,
    name   = as.character(row$Category),
    legendgroup = as.character(row$Category),
    showlegend  = (i == match(row$Category, timeline$Category)),
    hoverlabel = list(
      bgcolor = color,
      bordercolor = "white",
    ...
    hovertemplate = paste0(
      ...
      "From: ", format(row$Start, "%b %Y"),
      " \u2192 ", ifelse(is_active, "Present", format(row$End, "%b %Y")),
      "<extra></extra>"
    )
  )
}
