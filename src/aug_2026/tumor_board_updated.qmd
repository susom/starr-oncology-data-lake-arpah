...
tb_sql <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_person_metrics.sql"
tb_den<- fetch_data_from_sql_file(tb_sql, yaml_file_path, release = "aug_2026")
tb_den
...
## basic counts 
sql_thr<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/image_occ_thoracic/image_occ_person_thr.sql"
res_den_thr<- fetch_data_from_sql_file(sql_thr, yaml_file_path, release = "aug_2026")
head(res_den_thr)
```

As of August 2026, From `r format(res_den_thr$counts[res_den_thr$variable_name=="Number of unique persons with thoracic cancer"], big.mark = ",")` patients with thoracic cancer, `r format(tb_den$n_pts[1] , big.mark = ",")` also had a Tumor Board encounter. 



```{r time trend, message=FALSE, warning=FALSE, results='hide'}
# time trend
tb_time <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_over_time.sql"
dt <- fetch_data_from_sql_file(tb_time, yaml_file_path, release = "aug_2026")
head(dt)
```
...
```
```{r extra data, message=FALSE, warning=FALSE, include=FALSE}
library(bigrquery)
library(dplyr)

# Authenticate (opens browser first time, then caches)
bq_auth()
...
sql <- "
SELECT *
FROM `som-rit-phi-oncology-dev.Tumor_Board_Email.PatientList_from_TB_emails`
"

tb_data <- bq_project_query(project_id, sql) %>%
  bq_table_download()

tb_data <- tb_data %>%
  mutate(source = "tb_emails")

tb_email_summary <- tb_data %>%
  group_by(tb_year) %>%       # adjust to your actual date/year column name
  summarise(
    total_patients  = n_distinct(person_id),   # adjust to your actual patient ID column
    .groups = "drop"
  ) %>%
  mutate(
    data  = "tb_emails"  , year=tb_year,
     n_tb_encounters = NA_real_,   # no encounter data for this source
    ratio = NA_real_
  )

tb_email_summary=tb_email_summary %>% filter(tb_year<2018)
```
...
dt_combined <- bind_rows(dt, tb_email_summary)

dt_combined <- dt_combined %>%
  mutate(y_value = if_else(data == "tb_emails", total_patients, n_tb_encounters))

# Keep all point shapes the same except tb_emails, which gets a distinct marker.
shape_values <- setNames(rep(16, length(unique(dt_combined$data))), unique(dt_combined$data))
shape_values["tb_emails"] <- 17

p <- ggplot(dt_combined, aes(x = year, y = y_value, color = data, shape = data, group = data)) +
  geom_point(aes(
    text = paste0(
      "Year: ", year,
      "<br>n_encounters: ", if_else(is.na(n_tb_encounters), "NA", format(round(n_tb_encounters, 0), big.mark = ",")),
      "<br>n_patients: ", total_patients,
      if_else(!is.na(ratio), paste0("<br>ratio: ", round(ratio, 3)), "")
    )
  ), size = 3) +
  ...
  labs(
    ...
    x = "Year",
    y = "Number of Encounters / Patients",  # updated to reflect mixed metric
    color = NULL
  ) +
  theme(
...
**Update: Added TB Emails patient source to Tumor Board encounters plot**

This release adds a new data source to the "Tumor Board Encounters Over Time" plot: patients identified from internal Thoracic Tumor Board communications and dark-mining analysis of oncologist emails. This method surfaces Tumor Board patients not captured by the current structured-data definition.
Note that the number of encounters can be viewed by hovering on the datapoint for the other series, but this information is not available for the `tb_emails` series.
...
# Parameters
sql_overlap <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_overlap.sql"
res<- fetch_data_from_sql_file(sql_overlap, yaml_file_path, release = "aug_2026")
...
# Parameters
sql_demg <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_demographic_all.sql"
res <- fetch_data_from_sql_file(sql_demg, yaml_file_path, release = "aug_2026")
```
...
# Parameters
sql_case <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_case_status.sql"
case <- fetch_data_from_sql_file(sql_case, yaml_file_path, release = "aug_2026")
case
...
# Parameters
sql_img <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_imag_occ.sql"
tb_img<- fetch_data_from_sql_file(sql_img, yaml_file_path, release = "aug_2026")
tb_img
```
As of August 2026, `r format(tb_img$patient_count[1], big.mark = ",")` had image occurrence procedures.
...
sql_image_an <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_imag_occ_anatomic.sql"
res_ana <- fetch_data_from_sql_file(sql_image_an, yaml_file_path, release = "aug_2026")
head(res_ana)
...
# Parameters
sql_img_mod <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/tumor_board/feb26/tumor_board_thoracic_imag_occ_modality.sql"
tb_img_mod<- fetch_data_from_sql_file(sql_img_mod, yaml_file_path, release = "aug_2026")
tb_img_mod
```
...
### Source Codes:
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/aug_2026/tumor_board_updated.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/tumor_board/feb26). 
