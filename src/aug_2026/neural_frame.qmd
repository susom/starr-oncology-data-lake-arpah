...
## Neural Frame Event Table {.tabset}
...
treatment_overview <- event_summary %>%
  mutate(
    ...
    `Date Range` = paste0(format(earliest_event_date, "%Y-%m-%d"), " to ", format(latest_event_date, "%Y-%m-%d"))
  ) %>%
  select(`Total Cases`, `Total Events`, `Unique Event Types`, `Date Range`)

create_gt_table_v1(
  data = treatment_overview,
  columns = c("Total Cases", "Total Events", "Unique Event Types", "Date Range"),
  labels = c(
    "Total Cases" = "Cases",
    "Total Events" = "Events",
...
#### Treatment Modalities Distribution
...
plot_ly(
  ...
  y = ~case_count,
  type = 'bar',
  marker = list(color = c('#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00')),
...
```{r radiation therapy table, message=FALSE, warning=FALSE}
rad_table <- radiation_details %>%
  arrange(desc(cases_with_radiation)) %>%
  head(10) %>%
  mutate(
    `Modality` = eventphaseradiationtreatmentmodalitydescription,
    `Cases` = format(cases_with_radiation, big.mark = ","),
    `Avg Total Dose (Gy)` = round(avg_total_dose_gy, 2),
    `Avg Dose/Fraction (Gy)` = round(avg_dose_per_fraction_gy, 2),
    `Avg Fractions` = round(avg_number_of_fractions, 1)
  ) %>%
  select(`Modality`, `Cases`, `Avg Total Dose (Gy)`, `Avg Dose/Fraction (Gy)`, `Avg Fractions`)

create_gt_table_v1(
  data = rad_table,
  columns = c("Modality", "Cases", "Avg Total Dose (Gy)", "Avg Dose/Fraction (Gy)", "Avg Fractions"),
  subtitle_text = "Top 10 Radiation Therapy Modalities with Dosimetry Data"
)
```
...
This section covers treatment outcomes including completion status, recurrence patterns, and clinical follow-up.

#### Treatment Completion Status
```{r completion status table, message=FALSE, warning=FALSE}
completion_table <- completion_status %>%
  arrange(desc(case_count)) %>%
  mutate(
    `Chemotherapy Status` = ifelse(is.na(eventchemotherapycompletionstatus), "Not Available", eventchemotherapycompletionstatus),
    `Radiation Status` = ifelse(is.na(eventradiationcompletionstatus), "Not Available", eventradiationcompletionstatus),
    `Cases` = format(case_count, big.mark = ","),
    `% of Total` = paste0(round(percent_of_cases, 1), "%")
  ) %>%
  select(`Chemotherapy Status`, `Radiation Status`, `Cases`, `% of Total`)

create_gt_table_v1(
  data = completion_table,
  columns = c("Chemotherapy Status", "Radiation Status", "Cases", "% of Total"),
  subtitle_text = "Treatment Completion Status Summary"
)
```

#### Recurrence Patterns
```{r recurrence table, message=FALSE, warning=FALSE}
recurrence_table <- recurrence_data %>%
  arrange(desc(cases_with_recurrence)) %>%
  head(15) %>%
  mutate(
    `Recurrence Type` = eventtypeofrecurrencedescription,
    `Primary Site` = eventrecurrencesite1,
    `Cases` = format(cases_with_recurrence, big.mark = ","),
    `Patients` = format(patients_with_recurrence, big.mark = ",")
  ) %>%
  select(`Recurrence Type`, `Primary Site`, `Cases`, `Patients`)

create_gt_table_v1(
  data = recurrence_table,
  columns = c("Recurrence Type", "Primary Site", "Cases", "Patients"),
  subtitle_text = "Top 15 Recurrence Patterns"
)
```

#### Recurrence Distribution Visualization
```{r recurrence plot, message=FALSE, warning=FALSE}
recurrence_plot_data <- recurrence_data %>%
  arrange(desc(cases_with_recurrence)) %>%
  head(10) %>%
  mutate(recurrence_label = ifelse(is.na(eventtypeofrecurrencedescription), 
                                    "Unspecified", 
                                    eventtypeofrecurrencedescription))

plot_ly(
  data = recurrence_plot_data,
  x = ~reorder(recurrence_label, -cases_with_recurrence),
  y = ~cases_with_recurrence,
  type = 'bar',
  marker = list(color = '#d62728'),
  hovertemplate = ~paste0(
    "<b>", recurrence_label, "</b>",
    "<br>Cases: ", scales::comma(cases_with_recurrence),
    "<br>Patients: ", scales::comma(patients_with_recurrence),
    "<extra></extra>"
  )
) %>%
  layout(
    title = "Top 10 Recurrence Patterns",
    xaxis = list(title = "Recurrence Type"),
    yaxis = list(title = "Number of Cases", tickformat = ","),
    showlegend = FALSE,
    hovermode = 'x unified',
    margin = list(b = 120)
...
This section details diagnostic procedures, clinical trials, and other protocol-related events.
...
protocol_table <- protocol_analysis %>%
  arrange(desc(enrolled_patients)) %>%
  mutate(
    `Participation` = ifelse(is.na(eventprotocolparticipation), "Not Specified", eventprotocolparticipation),
    `Trial Type` = ifelse(is.na(eventprotocoltrialtype), "Not Specified", eventprotocoltrialtype),
    `Eligibility Status` = ifelse(is.na(eventprotocoleligibilitystatus), "Not Specified", eventprotocoleligibilitystatus),
    `Protocols` = format(unique_protocols, big.mark = ","),
    `Cases` = format(enrolled_cases, big.mark = ","),
    `Patients` = format(enrolled_patients, big.mark = ",")
  ) %>%
  select(`Participation`, `Trial Type`, `Eligibility Status`, `Protocols`, `Cases`, `Patients`)

create_gt_table_v1(
  data = protocol_table,
  columns = c("Participation", "Trial Type", "Eligibility Status", "Protocols", "Cases", "Patients"),
  subtitle_text = "Clinical Trial Participation and Enrollment"
)
```

#### Surgical Procedures Summary
```{r surgical procedures table, message=FALSE, warning=FALSE}
surgery_table <- surgery_details %>%
  arrange(desc(cases_with_surgery)) %>%
  head(12) %>%
  mutate(
    `Primary Site` = eventrxsummsurgprimsitedescription,
    `Approach` = eventrxsummsurgicalapprochdescription,
    `Cases` = format(cases_with_surgery, big.mark = ","),
    `Events` = format(surgical_events, big.mark = ",")
  ) %>%
  select(`Primary Site`, `Approach`, `Cases`, `Events`)

create_gt_table_v1(
  data = surgery_table,
  columns = c("Primary Site", "Approach", "Cases", "Events"),
  subtitle_text = "Top 12 Surgical Procedures by Primary Site and Approach"
)
```
