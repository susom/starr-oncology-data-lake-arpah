...
sql_den<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_den.sql"
nf_den <- fetch_data_from_sql_file(sql_den, yaml_file_path, release = "aug_2026")
nf_den
```

```{r case status all ,message=FALSE, warning=FALSE, results='hide'}
sql_nf<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_case_status.sql"
case <- fetch_data_from_sql_file(sql_nf, yaml_file_path, release = "aug_2026")
head(case)
...
sql_thr_case<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_case_status_person_metrics.sql"
thr_case <- fetch_data_from_sql_file(sql_thr_case, yaml_file_path, release = "aug_2026")
head(thr_case)
thr_case$flag="Thoracic Cancer"
thr_case$nfcasestatus[thr_case$nfcasestatus=="Incomplete - Edit"]<-"Incomplete"


sql_thr<-"/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_denominator.sql"

thr_den=fetch_data_from_sql_file(sql_thr, yaml_file_path, release = "aug_2026")
thr_den
```
```{r,message=FALSE, warning=FALSE, results='hide'}
# Calculate counts and percentages using unique person counts
...
sql_thr_c<-"/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_denominator_complete.sql"

thr_den_c=fetch_data_from_sql_file(sql_thr_c, yaml_file_path, release = "aug_2026")
thr_den_c
## denominators ##
sum_thr_c=thr_den_c$unique_thoracic_cancer_pts
sum_thr=thr_den$unique_thoracic_cancer_pts
```
As of August 2026, there are  `r format(thr_den$unique_thoracic_cancer_pts, big.mark = ",")` unique thoracic patients with `r format(case_thr$Percentage[case_thr$Status=="Completed"], big.mark = ",")` % completed cases. 
...
sql_thr_yr<- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_completed_cases_dx_year_person_metrics.sql"
thr_yr <- fetch_data_from_sql_file(sql_thr_yr, yaml_file_path, release = "aug_2026")
...
sql_thr_his <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_completed_cases_histology_person_metrics.sql"
thr_his <- fetch_data_from_sql_file(sql_thr_his, yaml_file_path, release = "aug_2026")
...
sql_stg <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_stage_person_metrics.sql"
thr_stg <- fetch_data_from_sql_file(sql_stg, yaml_file_path, release = "aug_2026")
thr_stg
...
```{r, message=FALSE, warning=FALSE, results='hide'}
sql_tx <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_completed_cases_tx_summary_person_metrics.sql"
thr_tx <- fetch_data_from_sql_file(sql_tx, yaml_file_path, release = "aug_2026")
...
recurrence <- read_csv("/workspaces/starr-oncology-data-lake-arpah/src/aug_2026/data/recurrenceType_category.csv")
...
sql_rec <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_recurrence.sql"
thr_rec <- fetch_data_from_sql_file(sql_rec, yaml_file_path, release = "aug_2026")
...
sql_soh <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_social_history.sql"
thr_soh <- fetch_data_from_sql_file(sql_soh, yaml_file_path, release = "aug_2026")
...
```{r , message=FALSE, warning=FALSE, results='hide'}
sql_sm_omop <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std_phi/thoracic/feb26/nf_thoracic/scr_thoracic_smoking_omop.sql"
thr_smk <- fetch_data_from_sql_file(sql_sm_omop, yaml_file_path, release = "aug_2026")
...
sql_treatment_summary <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/common/neural_frame/nf_treatment_modalities.sql"
treatment_summary <- fetch_data_from_sql_file(sql_treatment_summary, yaml_file_path, release = "aug_2026")
...
sql_protocol_analysis <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/common/neural_frame/nf_protocol_participation.sql"
protocol_analysis <- fetch_data_from_sql_file(sql_protocol_analysis, yaml_file_path, release = "aug_2026")

sql_diagnostics <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/common/neural_frame/nf_diagnostic_procedures.sql"
diagnostics_data <- fetch_data_from_sql_file(sql_diagnostics, yaml_file_path, release = "aug_2026")
...
temporal_plot <- temporal_data %>%
  ...
  filter(year >= 1950 & year <= 2026)

plot_ly(data = temporal_plot) %>%
  add_trace(
    x = ~year, y = ~distinct_patients,
    type = 'bar',
    name = 'Patients',
...
```{r treatment modalities table, message=FALSE, warning=FALSE}
treatment_table <- treatment_summary %>%
  arrange(desc(case_count)) %>%
  mutate(
    `Treatment Modality` = treatment_modality,
    `Cases` = format(case_count, big.mark = ","),
    `Patients` = format(patient_count, big.mark = ","),
    `% of Cases` = paste0(round(100 * case_count / sum(case_count), 1), "%")
  ) %>%
  select(`Treatment Modality`, `Cases`, `Patients`, `% of Cases`)

create_gt_table_v1(
  data = treatment_table,
  columns = c("Treatment Modality", "Cases", "Patients", "% of Cases"),
  labels = c(
  ...
  subtitle_text = "Distribution of Primary Treatment Modalities"
)
```

#### Treatment Modalities Visualization
```{r treatment modalities plot, message=FALSE, warning=FALSE}
plot_ly(
  data = treatment_summary %>% arrange(desc(case_count)),
  x = ~reorder(treatment_modality, -case_count),
  y = ~case_count,
  type = 'bar',
  marker = list(color = RColorBrewer::brewer.pal(max(3, nrow(treatment_summary)), "Set3")),
  hovertemplate = ~paste0(
    "<b>", treatment_modality, "</b>",
    "<br>Cases: ", scales::comma(case_count),
    "<br>Patients: ", scales::comma(patient_count),
    "<extra></extra>"
  )
  ...
  layout(
    ...
    yaxis = list(title = "Number of Cases", tickformat = ","),
    showlegend = FALSE,
    hovermode = 'x unified',
    margin = list(b = 100)
  )
...
chemo_table <- chemo_courses %>%
  arrange(eventcoursenumber) %>% filter(patients_receiving_chemo > 5 )%>%
  mutate(
...
```{r radiation therapy table, message=FALSE, warning=FALSE, include=FALSE}
rad_table <- radiation_details %>%
  arrange(desc(cases_with_radiation)) %>%
  head(15) %>%
  mutate(
    `Modality` = radiation_modality,
    `Cases` = format(cases_with_radiation, big.mark = ","),
    `Patients` = format(patients_with_radiation, big.mark = ","),
    `Avg Dose (Gy)` = ifelse(is.na(avg_total_dose_gy), "—", as.character(avg_total_dose_gy)),
    `Avg Fractions` = ifelse(is.na(avg_number_of_fractions), "—", as.character(avg_number_of_fractions))
  ) %>%
  select(`Modality`, `Cases`, `Patients`, `Avg Dose (Gy)`, `Avg Fractions`)

create_gt_table_v1(
  data = rad_table,
  columns = c("Modality", "Cases", "Patients", "Avg Dose (Gy)", "Avg Fractions"),
  labels = c(
    "Modality" = "Radiation Modality",
    "Cases" = "Cases",
    "Patients" = "Patients",
    "Avg Dose (Gy)" = "Avg Total Dose (Gy)",
    "Avg Fractions" = "Avg Fractions"
  ),
  subtitle_text = "Radiation Therapy by Modality"
)
```
...
```{r completion status table, message=FALSE, warning=FALSE, include=FALSE}
comp_table <- completion_status %>%
  arrange(desc(case_count)) %>%
  mutate(
    `Chemo Status` = ifelse(is.na(eventchemotherapycompletionstatus), "Not Applicable", eventchemotherapycompletionstatus),
    `Radiation Status` = ifelse(is.na(eventradiationcompletionstatus), "Not Applicable", eventradiationcompletionstatus),
    `Cases` = format(case_count, big.mark = ","),
    `% of Cases` = paste0(percent_of_cases, "%")
  ) %>%
  select(`Chemo Status`, `Radiation Status`, `Cases`, `% of Cases`)

create_gt_table_v1(
  data = comp_table,
  columns = c("Chemo Status", "Radiation Status", "Cases", "% of Cases"),
  labels = c(
    "Chemo Status" = "Chemotherapy Completion",
    "Radiation Status" = "Radiation Completion",
    "Cases" = "Cases",
    "% of Cases" = "Percentage"
  ),
  subtitle_text = "Treatment Completion Status"
)
```
...
```{r diagnostic procedures table, message=FALSE, warning=FALSE}
diag_table <- diagnostics_data %>%
  arrange(desc(cases_with_procedure)) %>%
  head(15) %>%
  mutate(
    `Procedure` = eventdiagnosticproceduredescription,
    `Cases` = format(cases_with_procedure, big.mark = ","),
    `Patients` = format(patients_with_procedure, big.mark = ","),
    `Events` = format(procedure_events, big.mark = ",")
  ) %>%
  select(`Procedure`, `Cases`, `Patients`, `Events`)

create_gt_table_v1(
  data = diag_table,
  columns = c("Procedure", "Cases", "Patients", "Events"),
  labels = c(
    "Procedure" = "Procedure",
    "Cases" = "Cases",
    "Patients" = "Patients",
  ...
  subtitle_text = "Top 15 Diagnostic and Pathology Procedures"
)
```

### ARIA Radiation Logistics

ARIA (Radiation Oncology Information System) provides per-phase radiation treatment logistics that are not available from general NF abstracting. These include target volumes, lymph node involvement, daily/total dosing, fraction counts, and treatment end dates. The following metrics are restricted to radiation events only (event types T06 and P-T06).
...
create_gt_table_v1(
  ...
  labels = c(
    "Patients" = "Patients with ARIA Data",
    "Cases" = "Cases with ARIA Data",
    "Events" = "Events with ARIA Data"
  ),
  subtitle_text = "Patients with ARIA Phase-Level Radiation Logistics"
)
```
...
```{r aria detail plot, message=FALSE, warning=FALSE}
plot_ly(
  data = aria_detail %>% arrange(desc(patients)),
  y = ~reorder(aria_field, patients),
  x = ~patients,
  type = 'bar',
  orientation = 'h',
  ...
  layout(
    title = "ARIA Phase-Level Field Coverage (Patients)",
...
The source codes for this page can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/aug_2026/neural_frame.qmd), and the sql queries that support the metrics can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/neural_frame). 
The queries regarding thoracic cancer can be found [here](https://github.com/susom/starr-oncology-data-lake-arpah/tree/main/src/sql_std_phi/thoracic/feb26/nf_thoracic).
