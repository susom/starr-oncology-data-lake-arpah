...
```{r death_source_overlap_table, message=FALSE, warning=FALSE}
...
```{r death_overlap_combo_plot, message=FALSE, warning=FALSE}
library(plotly)
library(dplyr)

# Stacked / grouped bar of mutually exclusive combinations
combo_data <- death_overlap_res %>%
  filter(!death_source %in% c("OMOP death", "NeuralFrame death", "External death")) %>%
  arrange(desc(n_total))

plot_ly(
  data = combo_data,
  x    = ~reorder(death_source, n_total),
  y    = ~n_total,
  type = 'bar',
  marker = list(color = '#80b1d3'),
  text = ~paste0(
    death_source,
    "<br>Patients: ", scales::comma(n_total),
    "<br>% of total: ", pct_overlap_with_ext_death, "%"
  ),
  hoverinfo = "text"
) %>%
  layout(
    title  = "Mutually Exclusive Death Source Combinations (August 2026)",
