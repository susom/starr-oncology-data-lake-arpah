...
### Death Counts Over Time (OMOP + NeuralFrame)
...
```{r death_source_overlap, message=FALSE, warning=FALSE, results='hide'}
...
overlap_combos <- death_overlap_res %>%
  filter(!death_source %in% c("OMOP death", "NeuralFrame death", "External death")) %>%
...
### Mutually Exclusive External Death Source Combinations

```{r death_ext_sources, message=FALSE, warning=FALSE}
library(plotly)
library(dplyr)
...
plot_ly(
  ...
  layout(
    title  = "Unique Patients by Mutually Exclusive External Death Source Combination (August 2026)",
