...
**Analysis Data Sources:** This analysis reflects the PHI scrubbed data from February to November 2025.
...
yaml_file_path <- "/workspaces/starr-oncology-data-lake-arpah/src/deid_nov_2025/sql_params.yml"
```
...
# Use common SQL file for person_metrics (identical across all releases)
# Single file path since this query is universal
person_sql_path <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/common/arpah_cohort/person_metrics.sql"

person_res <- run_monthly_metrics(
  datasets_by_month = datasets_by_month,
  sql_file_path = person_sql_path  # Note: singular - uses common file!
)
```
...
# Use common Philips denominator SQL (identical across May/Aug/Nov)
# Single file path since this query is universal for all Philips releases
philips_denom_sql_path <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/philips/philips_denominator.sql"

philips_denom <- run_monthly_metrics(
  datasets_by_month = datasets_by_month[c("may_2025", "aug_2025", "nov_2025")],
  sql_file_path = philips_denom_sql_path,
  release_to_month = release_to_month
)
```

```{r philips_tests, message=FALSE, warning=FALSE, results='hide'}
# Use common Philips tests ordered SQL (identical across May/Aug/Nov)
# Single file path since this query is universal for all Philips releases
philips_tests_sql_path <- "/workspaces/starr-oncology-data-lake-arpah/src/sql_std/philips/philips_tests_ordered.sql"

philips_tests <- run_monthly_metrics(
  datasets_by_month = datasets_by_month[c("may_2025", "aug_2025", "nov_2025")],
  sql_file_path = philips_tests_sql_path,
  release_to_month = release_to_month
)
```
