...
scr_thoracic_patients as (
    select distinct
person_id, cleaned_nf_mrn,cleaned_nf_dob
    FROM scr
    WHERE trim(medicalRecordNumber) <> '' and length(dateOfBirth) = 8
    and lower(primarysiteDescription) like '%lung%'
    or lower(primarysiteDescription) like '%bronchus%'
    or lower(primarysiteDescription) like '%thymus%'
    GROUP BY person_id, cleaned_nf_mrn, cleaned_nf_dob --return results at patient level
)
select count(distinct person.person_id) n_pts
from
