...
select distinct p.person_id , nf.person_id 
...
,
coding as (
select stanford_hgvs_coding, count(distinct(person_id)) as n_pts, 'hgvs_coding' as flag
...
group by 1),
protein as (
  select stanford_hgvs_protein, count(distinct(person_id)) as n_pts, 'hgvs_protein' as flag
from `@oncology_prod.@oncology_philips.onc_philips_mtb_aberrations` abr
inner join thoracic_pts using(person_id)
where hgnc_gene in ("KRAS") and stanford_hgvs_protein is not null
group by 1),
change_aa as (select stanford_aa_change, count(distinct(person_id)) as n_pts, 'amino_acid_changes' as flag
from `@oncology_prod.@oncology_philips.onc_philips_mtb_aberrations` abr
inner join thoracic_pts using(person_id)
