...
credentials_path <- "/home/rstudio/.config/gcloud/application_default_credentials.json"
folder_path <- "../sql/arpah_clinical"
...
```{r, message=FALSE, warning=FALSE, results='hide'}
credentials_path <- "/home/rstudio/.config/gcloud/application_default_credentials.json"
folder_path_tb <- "../sql/tumor_board"
project_name = "som-rit-phi-oncology-prod"
res_tb <- fetch_data_from_sql_yml2(credentials_path, project_name, folder_path_tb)
```
```{r, message=FALSE, warning=FALSE, results='hide'}
colnames(res_arpah)<-c("pt_count_arph", "file_name")

res_tb$file_name=gsub("tumor_board_","", res_tb$sql_file_name)
colnames(res_tb)[1]<-"pt_count_tb"

# Ensure res_arpah has the file_name column
if (!"file_name" %in% colnames(res_arpah)) {
  stop("The column 'file_name' is not present in res_arpah")
}
...
```{r, message=FALSE, warning=FALSE, results='hide'}
## check
res_dat=res_arpah %>% inner_join(res_tb, by= "file_name")
print(res_dat)
```
```{r, message=FALSE, warning=FALSE, results='hide'}
res_dat <- res_dat %>%
  mutate(
    metric = case_when(
      file_name == "note_path_cyto_person_metrics.sql" ~ "Number of patients with pathology/cytology clinical report",
      file_name == "note_imaging_person_metrics.sql" ~ "Number of patients with imaging reports, also a proxy for radiology imaging data",
      file_name == "vital_status_person_metrics.sql" ~ "Number of patients with present Hospital or Stanford Cancer Registry death date",
      file_name == "omop_chemo_person_metrics.sql" ~ "Number of patients given chemo medication",
      file_name == "wsi_beaker_path_person_metrics.sql" ~ "Number of patients with Pathology reports",
      TRUE ~ NA_character_
    )
  )
print(res_dat)
...
```{r, message=FALSE, warning=FALSE, results='hide'}
credentials_path <- "/home/rstudio/.config/gcloud/application_default_credentials.json"
folder_path_tc <- "../sql/scr/thoracic"
project_name = "som-rit-phi-oncology-prod"
...
```{r, message=FALSE, warning=FALSE}
create_gt_table2(res_rc, recurrencetype1stdescription, n_pts, "Recurrence Type - Neural Frame Outcomes", "Patient Count", "Number of Neural Frame Patients (196,564)")
```
